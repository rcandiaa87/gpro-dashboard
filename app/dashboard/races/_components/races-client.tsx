...
import { Flag, Trophy, Clock, MapPin, ChevronRight, Loader2 } from 'lucide-react';
...
export function RacesClient() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/gpro/seasons?idm=${idm}`)
      .then(r => r.json())
      .then((d: any) => { if (d?.[0]?.temporada && !season) setSeason(d[0].temporada); })
      .catch(() => {});
  }, [idm]);
  ...
  useEffect(() => {
    ...
    fetch(`/api/gpro/races?idm=${idm}&season=${season}`)
      .then(r => r.json())
      .then((d: any) => setRaces(Array.isArray(d) ? d : []))
      .catch(() => setRaces([]))
