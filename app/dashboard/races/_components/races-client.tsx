...
export function RacesClient() {
  ...
  const loadRaces = useCallback(() => {
    ...
    setFetchError(false);
    const controller = new AbortController();
    fetch(`${basePath}/api/gpro/races?idm=${idm}&season=${season}`, { signal: controller.signal })
      .then(r => { if (!r.ok) throw new Error(`HTTP ${r.status}`); return r.json(); })
    ...
    <div className="p-4 lg:p-6 space-y-6">
      ...
      {loading ? (
        <div className="flex items-center justify-center h-64">
          <div role="status" aria-label="Cargando historial de carreras">
        ...
        <div className="flex flex-col items-center justify-center h-64 gap-4 text-slate-400">
          ...
          <button onClick={loadRaces} className="px-4 min-h-[44px] bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-lg text-sm border border-slate-600 transition-colors">
            Reintentar
          </button>
        </div>
      ) : races?.length === 0 ? (
        <div className="flex items-center justify-center h-64 text-slate-400"><p>No hay carreras para esta temporada</p></div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {races.map((race) => (
            <Link key={`${race.temporada}-${race.carrera}`}
              href={`/dashboard/races/${race.temporada}/${race.carrera}?idm=${idm}`}
              className="bg-slate-900/80 border border-slate-700/50 rounded-xl p-5 hover:bg-slate-800/80 hover:border-blue-500/30 transition-colors group">
              <div className="flex items-start justify-between mb-3">
                <div>
                  <p className="text-sm text-slate-400">S{race.temporada} R{race.carrera}</p>
                  <p className="text-lg font-bold text-white mt-1">{race.trackName ?? 'Circuito'}</p>
                  <p className="text-xs text-slate-400 mt-0.5">{race.group ?? ''}</p>
                </div>
